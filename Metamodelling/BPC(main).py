...
iteration = 0
stop = 0
StoremdMinusEta = np.zeros((30,M))# Generate an empty 2D 30*M array, 30 is the max number of iteration, check if need to increase it
...
with open('BPC_data.dat', 'wb') as output:
    obj1 = Data(D, Y, P, md, check3, MaxIntOrder, iteration, TotalIndices)
    pickle.dump(obj1, output, pickle.HIGHEST_PROTOCOL)

    obj2 = Data(D, Y, P, md, check3, MaxIntOrder, iteration, TotalIndices)
    pickle.dump(obj2, output, pickle.HIGHEST_PROTOCOL)

#----------------------------------------------
    # To load data from a file

with open('company_data.dat', 'rb') as input:
    obj1 = pickle.load(input)
    print(obj1.D)  # -> D
    .
    .
    .
    print(obj1.TotalIndices)  # -> TotalIndices
    ...
    .
    .
    print(obj2.TotalIndices)  # -> TotalIndices       
